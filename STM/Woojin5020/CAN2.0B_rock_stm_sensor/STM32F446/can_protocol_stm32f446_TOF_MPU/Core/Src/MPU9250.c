...
MPU9250_Result_t MPU9250_Init(MPU9250_t *MPU9250, MPU9250_Device_t dev, MPU9250_Accel_Scale_t accScale, MPU9250_Gyro_Scale_t gyroScale, MPU9250_Mag_Scale_t magScale)
{
	...
	if (isMPU9250Ready(&hi2c2, MPU9250->I2C_Addr) != MPU9250_RESULT_OK)
		return MPU9250_RESULT_NC;

	/* Who Am I Check */
	readByte(&hi2c2, MPU9250->I2C_Addr, WHO_AM_I, &data);
	if (data != 0x71)
		return MPU9250_RESULT_NC;

	/* Device Wake Up */
	writeByte(&hi2c2, MPU9250->I2C_Addr, PWR_MGMT_1, 0x00);
	HAL_Delay(100);

	/* Auto-select best (stable) available clock source */
	writeByte(&hi2c2, MPU9250->I2C_Addr, PWR_MGMT_1, 0x01);
	HAL_Delay(200);
	 ...
	 */
	writeByte(&hi2c2, MPU9250->I2C_Addr, CONFIG, 0x03);

	/* Sample Rate = Fs / (1 + SMPLRT_DIV)
	 * @Sample Rate = 1000 / (1 + 4) = 200 Hz
	 */
	writeByte(&hi2c2, MPU9250->I2C_Addr, SMPLRT_DIV, 0x04);

	/* Gyro Configuration */
	readByte(&hi2c2, MPU9250->I2C_Addr, GYRO_CONFIG, &data);
	data &= ~0x02;		/* [1:0] - Clear Fchoice_b[1:0] bits */
	data &= ~0x18;		/* [4:3] - Clear GYRO_FS_SEL[1:0] bits */
	data |= gyroScale;
	writeByte(&hi2c2, MPU9250->I2C_Addr, GYRO_CONFIG, data);

	/* Accel Configuraiton */
	readByte(&hi2c2, MPU9250->I2C_Addr, ACCEL_CONFIG, &data);
	data &= ~0x18;		/* [4:3] - Clear ACCEL_FS_SEL[1:0] bits */
	data |= accScale;
	writeByte(&hi2c2, MPU9250->I2C_Addr, ACCEL_CONFIG, data);

	/* Accel_2 Configuration
	 * @Accel BW: 44.8 Hz
	 */
	readByte(&hi2c2, MPU9250->I2C_Addr, ACCEL_CONFIG_2, &data);
	data &= ~0x0F;		/* Clear ACCEL_CONFIG_2[3:0] Bits */
	data |= 0x03;		/* A_DLPFCFG[2:0] bits are set to 011 */
	writeByte(&hi2c2, MPU9250->I2C_Addr, ACCEL_CONFIG_2, data);

	/* Interrupt Pin/Bypass Pin Configuration
	 * @INT Pin: push-pull (active high)
	 * Interrupt on raw sensor data ready
	 */
	writeByte(&hi2c2, MPU9250->I2C_Addr, INT_PIN_CFG, 0x22);
	writeByte(&hi2c2, MPU9250->I2C_Addr, INT_ENABLE, 0x01);

	/* Magnetometer Device Connection Check */
	if (isMPU9250Ready(&hi2c2, MPU9250->I2C_Addr_Mag) != MPU9250_RESULT_OK)
		return MPU9250_RESULT_NC;

	/* Magnetometer Power Down */
	writeByte(&hi2c2, MPU9250->I2C_Addr_Mag, CNTL, 0x00);
	HAL_Delay(10);
	/* Magnetometer Fuse ROM Access Mode ON */
	writeByte(&hi2c2, MPU9250->I2C_Addr_Mag, CNTL, 0x0F);
	HAL_Delay(10);
	/* Magnetometer Power Down */
	writeByte(&hi2c2, MPU9250->I2C_Addr_Mag, CNTL, 0x00);
	HAL_Delay(10);
	 ...
	 */
	writeByte(&hi2c2, MPU9250->I2C_Addr_Mag, CNTL, (1 << 4) | 2);
	HAL_Delay(10);
...
MPU9250_Result_t MPU9250_ReadAcc(MPU9250_t *MPU9250)
{
	...
	readMultiBytes(&hi2c2, MPU9250->I2C_Addr, ACCEL_XOUT_H, data, 6);
...
MPU9250_Result_t MPU9250_ReadGyro(MPU9250_t *MPU9250)
{
	...
	readMultiBytes(&hi2c2, MPU9250->I2C_Addr, GYRO_XOUT_H, data, 6);
...
MPU9250_Result_t MPU9250_ReadMag(MPU9250_t *MPU9250)
{
	...
	readByte(&hi2c2, MPU9250->I2C_Addr_Mag, ST1, &check);

	if (check & 0x01)
	{
		readMultiBytes(&hi2c2, MPU9250->I2C_Addr_Mag, HXL, data, 7);
...
MPU9250_Result_t MPU9250_ReadTemperature(MPU9250_t *MPU9250)
{
	...
	readMultiBytes(&hi2c2, MPU9250->I2C_Addr, TEMP_OUT_H, data, 2);
...
{
	uint8_t data;
	readByte(&hi2c2, MPU9250->I2C_Addr, INT_STATUS, &data);
	if (data & 0x01)
		return MPU9250_RESULT_OK;
...
HAL_StatusTypeDef writeByte(I2C_HandleTypeDef *hi2c2, uint8_t device_addr, uint8_t register_addr, uint8_t data)
{
	uint8_t buffer[2];
	buffer[0] = register_addr;
	buffer[1] = data;

	if (HAL_I2C_Master_Transmit(hi2c2, (uint16_t)device_addr, (uint8_t *)buffer, 2, 1000) != HAL_OK)
	{
...
HAL_StatusTypeDef readByte(I2C_HandleTypeDef *hi2c2, uint8_t device_addr, uint8_t register_addr, uint8_t *data)
{
	/* Transmit Register Address */
	if (HAL_I2C_Master_Transmit(hi2c2, (uint16_t)device_addr, &register_addr, 1, 1000) != HAL_OK)
	{
		{
			if (HAL_I2C_GetError(hi2c2) != HAL_I2C_ERROR_AF) {}
			return HAL_ERROR;
	...
	if (HAL_I2C_Master_Receive(hi2c2, (uint16_t)device_addr, data, 1, 1000) != HAL_OK)
	{
		{
			if (HAL_I2C_GetError(hi2c2) != HAL_I2C_ERROR_AF) {}
			return HAL_ERROR;
...
HAL_StatusTypeDef writeMultiBytes(I2C_HandleTypeDef *hi2c2, uint8_t device_addr, uint8_t register_addr, uint8_t *data, uint16_t count)
{
	if (HAL_I2C_Mem_Write(hi2c2, (uint16_t)device_addr, register_addr, register_addr > 0xFF ? I2C_MEMADD_SIZE_16BIT : I2C_MEMADD_SIZE_8BIT, data, count, 1000) != HAL_OK)
	{
		{
			if (HAL_I2C_GetError(hi2c2) != HAL_I2C_ERROR_AF) {}
			return HAL_ERROR;
...
HAL_StatusTypeDef readMultiBytes(I2C_HandleTypeDef *hi2c2, uint8_t device_addr, uint8_t register_addr, uint8_t *data, uint16_t count)
{
	/* Transmit Register Address */
	if (HAL_I2C_Master_Transmit(hi2c2, (uint16_t)device_addr, &register_addr, 1, 1000) != HAL_OK)
	{
	...
	if (HAL_I2C_Master_Receive(hi2c2, (uint16_t)device_addr, data, count, 1000) != HAL_OK)
	{
		if (HAL_I2C_GetError(hi2c2) != HAL_I2C_ERROR_AF) {}
		return HAL_ERROR;
	}
...
MPU9250_Result_t isMPU9250Ready(I2C_HandleTypeDef *hi2c2, uint8_t device_addr)
{
	/* Checks if device is ready to communicate */
	if (HAL_I2C_IsDeviceReady(hi2c2, (uint16_t)device_addr, 2, 5) != HAL_OK)
		return MPU9250_RESULT_NC;

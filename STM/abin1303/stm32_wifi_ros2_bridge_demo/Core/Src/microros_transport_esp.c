...
static void flush_esp(uint32_t ms)
{
    ...
    {
        if (HAL_UART_Receive(&huart6, &ch, 1, 5) != HAL_OK) break;
    }
}
...
{
    flush_esp(10);
    uart_send_str(&huart6, cmd);
    uart_send_str(&huart6, "\r\n");
...
static int recv_ipd(uint8_t* out, size_t want_len, uint32_t overall_timeout_ms)
{
    ...
    while (total < want_len && (HAL_GetTick() - t0) < overall_timeout_ms)
    {
        ...
        {
            if (HAL_UART_Receive(&huart6, &ch, 1, 10) != HAL_OK) continue;
        ...
        {
            if (HAL_UART_Receive(&huart6, &ch, 1, 50) != HAL_OK) continue;
            if (ch == ':') break;
